  ...
  el.innerHTML = `
    <div class="bg-panel border border-border rounded-xl p-5 lg:sticky lg:top-24">
      ...
      ${detailPayMode === 'credit' && creditKnown
        ...
        : ''}
      <button id="detailCallBtn" class="w-full flex items-center justify-center gap-2 bg-accent text-ink font-semibold text-[13.5px] rounded-lg py-3 mt-3 hover:bg-lime-300 transition disabled:opacity-40 disabled:cursor-not-allowed" ${(!s.active) || (detailPayMode === 'credit' && creditKnown && !canCredit) ? 'disabled' : ''}>
        ${detailPayMode === 'direct' ? `Pay ${fmtX1T(s.pricePerCall)} X1T and call` : 'Call using credit'} ${svg('arrow', 15, 2.2)}
      </button>
